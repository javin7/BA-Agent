...
export default function DocumentViewer({ document }) {
  const handleDownload = () => {
    const reqs = document.requirements && document.requirements.length > 0 ? document.requirements.map((r, i) => `${i + 1}. ${r}`).join('\n') : "None yet.";
    const ucs = document.useCases && document.useCases.length > 0 ? document.useCases.map((u, i) => {
      if (typeof u === 'string') return `${i + 1}. ${u}`;
      let text = `### ${i + 1}. ${u.title}\n**Description**: ${u.description || 'N/A'}\n`;
      if (u.preconditions && u.preconditions.length) text += `**Preconditions**: ${u.preconditions.join(', ')}\n`;
      if (u.postconditions && u.postconditions.length) text += `**Postconditions**: ${u.postconditions.join(', ')}\n`;
      if (u.scenarios && u.scenarios.length) text += `**Scenarios**:\n${u.scenarios.map(s => `  - ${s}`).join('\n')}\n`;
      return text;
    }).join('\n\n') : "None yet.";
    
    const actorsStr = document.actors && document.actors.length > 0 ? document.actors.join(', ') : 'None identified';
